...
export async function AuthMiddleware(permissionName, IgnoreMethods = []) {
  ...
  try {
    ...
    const permissionId = data.rows[0].id;
    ...
    return async (req, res, next) => {
      ...
      const { data } = await decodeToken(req.cookies.authorization.split(" ")[1]);
      ...
      if(!permissions) return res.status(401).send("No tienes permiso para acceder a esta ruta");
      ...
      if (!permissions?.map(x => x.id)?.includes(permissionId)) {
        ...
        const permissionUserData = permissions.find(x => x.id === permissionId);

        switch (req.method) {
          case "GET":
            if(permissionUserData.to.get || IgnoreMethods.includes("GET")) return next();
            return res.status(401).send("No tienes permiso para acceder a esta ruta");
          case "POST":
            if(permissionUserData.to.insert || IgnoreMethods.includes("POST")) return next();
            return res.status(401).send("No tienes permiso para acceder a esta ruta");
          case "PATCH":
            if(permissionUserData.to.update || IgnoreMethods.includes("PATCH")) return next();
            return res.status(401).send("No tienes permiso para acceder a esta ruta");
          case "DELETE":
            if(permissionUserData.to.delete || IgnoreMethods.includes("DELETE")) return next();
            return res.status(401).send("No tienes permiso para acceder a esta ruta");
          default:
            return res.status(401).send("No tienes permiso para acceder a esta ruta");
        }
